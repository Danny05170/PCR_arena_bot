...
PRIV_TIP = f'群員={priv.NORMAL} GameMaster={priv.SUPERUSER}'
...
def svs_to_msg(svs, gid, verbose_all, only_hidden, bc=False):
    msg = [f"群{gid}推送服務一覽："] if bc else [f"群{gid}服務一覽："]
...
@sv.on_prefix(('lssv','服務列表', '功能列表'), only_to_me=False)
...
@sv.on_prefix(('lsbcsv','推送服務列表', '推送列表'), only_to_me=False)
...
@sv.on_prefix(('enable', '啟用', '開啟', '打開'), only_to_me=False)
async def enable_service(bot:KokkoroBot, ev):
    await switch_service(bot, ev, turn_on=True)

@sv.on_prefix(('disable', '禁用', '關閉'), only_to_me=False)
...
async def switch_service(bot, ev:EventInterface , turn_on:bool):
    action_tip = '啟用' if turn_on else '禁用'

    names = ev.get_param().remain.split()
    if not names:
        await bot.kkr_send(ev, f"空格後接要{action_tip}的服務名", at_sender=True)
        return
    ...
    for name in names:
        if name in svs:
            ...
            if u_priv >= sv.manage_priv:
                ...
                try:
                    await bot.kkr_send(ev, f'權限不足！{action_tip}{name}需要：{sv.manage_priv}，您的：{u_priv}\n{PRIV_TIP}', at_sender=True)
                except:
                    pass
        else:
    ...
    msg = []
    if succ:
        msg.append(f'已{action_tip}服務：' + ', '.join(succ))
    if notfound:
        msg.append('未找到服務：' + ', '.join(notfound))
    if msg:
